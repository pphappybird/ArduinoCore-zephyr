/*
 * Copyright (c) Arduino s.r.l. and/or its affiliated companies
 *
 * SPDX-License-Identifier: Apache-2.0
 */

/**
 * @file nnlite_hello_world.c
 * @brief NNLite (Infineon ML Middleware) backend for the TFLM hello_world
 *        example. Loads the same TFLite flatbuffer used by the CPU-only
 *        hello_world example and runs it on the NNLite NPU via
 *        mtb_ml_model_init()/mtb_ml_model_run() instead.
 *
 * Model bytes are duplicated here (not shared with the sketch) because this
 * file is compiled into the statically-linked loader, not the LLEXT sketch.
 * Requires the ml-middleware/ml-tflite-micro west modules plus the
 * CONFIG_ML_MIDDLEWARE_* Kconfig options (see the board .conf).
 */

#include "nnlite_hello_world.h"

#include <errno.h>
#include <stdbool.h>

#include "mtb_ml.h"

/** NPU IRQ priority passed to mtb_ml_init(); ignored if no NPU component is
 *  compiled in. */
#define NNLITE_HELLO_WORLD_IRQ_PRIORITY (5U)

/** Arena size estimate for this model (the CPU interpreter path used a 2000
 *  byte tensor_arena for the same model). Re-tune based on the actual
 *  mtb_ml_model_t::buffer_size reported after init if this is too small. */
#define NNLITE_HELLO_WORLD_ARENA_SIZE (2048)

/* Standard TFLite flatbuffer for the hello_world sine model - identical
 * content to Documents/Arduino/libraries/Arduino_TensorFlowLite/examples/
 * hello_world/model.cpp's g_model[]. */
static const uint8_t g_model[] __attribute__((aligned(16))) = {
	
};

static const mtb_ml_model_bin_t g_model_bin = {
	.model_bin  = g_model,
	.model_size = sizeof(g_model),
	.arena_size = NNLITE_HELLO_WORLD_ARENA_SIZE,
};

static mtb_ml_model_t *g_model_obj;
static bool g_initialized;

int nnlite_hello_world_init(float *input_scale, int32_t *input_zero_point,
			     float *output_scale, int32_t *output_zero_point)
{
	cy_rslt_t result;

	if ((input_scale == NULL) || (input_zero_point == NULL) ||
	    (output_scale == NULL) || (output_zero_point == NULL)) {
		return -EINVAL;
	}

	if (!g_initialized) {
		result = mtb_ml_init(NNLITE_HELLO_WORLD_IRQ_PRIORITY);
		if (result != MTB_ML_RESULT_SUCCESS) {
			return -EIO;
		}

		/* buffer == NULL: let the middleware allocate its own
		 * persistent/scratch buffers sized from g_model_bin.arena_size. */
		result = mtb_ml_model_init(&g_model_bin, NULL, &g_model_obj);
		if (result != MTB_ML_RESULT_SUCCESS) {
			(void)mtb_ml_deinit();
			return -EINVAL;
		}

		g_initialized = true;
	}

	*input_scale = g_model_obj->input_scale;
	*input_zero_point = g_model_obj->input_zero_point;
	*output_scale = g_model_obj->output_scale;
	*output_zero_point = g_model_obj->output_zero_point;

	return 0;
}

int nnlite_hello_world_infer(int8_t x_quantized, int8_t *y_quantized)
{
	cy_rslt_t result;
	MTB_ML_DATA_T input_buf = (MTB_ML_DATA_T)x_quantized;

	if (!g_initialized) {
		return -ENODEV;
	}

	if (y_quantized == NULL) {
		return -EINVAL;
	}

	result = mtb_ml_model_run(g_model_obj, &input_buf);
	if (result != MTB_ML_RESULT_SUCCESS) {
		return -EIO;
	}

	*y_quantized = (int8_t)g_model_obj->output[0];

	return 0;
}
