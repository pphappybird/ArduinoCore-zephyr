...
/**
 * @file nnlite_micro_speech.c
 * @brief NNLite (Infineon ML Middleware) backend for the TFLM micro_speech
 *        example. Loads the same TFLite flatbuffer used by the CPU-only
 *        micro_speech example and runs it on the NNLite NPU via
 *        mtb_ml_model_init()/mtb_ml_model_run() instead.
 *
 * Model bytes are duplicated here (not shared with the sketch) because this
 * file is compiled into the statically-linked loader, not the LLEXT sketch.
 * Requires the ml-middleware/ml-tflite-micro west modules plus the
 * CONFIG_ML_MIDDLEWARE_* Kconfig options (see the board .conf).
 */
...
/** NPU IRQ priority passed to mtb_ml_init(); ignored if no NPU component is
 *  compiled in. */
#define NNLITE_MICRO_SPEECH_IRQ_PRIORITY (5U)

/** Arena size for this model. Measured actual usage via
 *  mtb_ml_model_t::buffer_size is 6980 bytes; 8 KiB leaves headroom without
 *  wasting the shared libc malloc arena the TFLM microfrontend also needs. */
#define NNLITE_MICRO_SPEECH_ARENA_SIZE (8 * 1024)

/* Standard TFLite flatbuffer for the micro_speech keyword-spotting model -
 * identical content to Documents/Arduino/libraries/Arduino_TensorFlowLite/
 * examples/micro_speech/micro_features_model.cpp's g_model[]. */
static const uint8_t g_model[] __attribute__((aligned(16))) = {
...
static const mtb_ml_model_bin_t g_model_bin = {
	.model_bin  = g_model,
	.model_size = sizeof(g_model),
	.arena_size = NNLITE_MICRO_SPEECH_ARENA_SIZE,
};

static mtb_ml_model_t *g_model_obj;
			      ...
			      float *output_scale, int32_t *output_zero_point)
{
	...
	if ((input_scale == NULL) || (input_zero_point == NULL) ||
	    (output_scale == NULL) || (output_zero_point == NULL)) {
		return -EINVAL;
	}

	if (!g_initialized) {
        /** Keep initialization free of blocking diagnostics; PDM capture is
         * timing-sensitive. */
		result = mtb_ml_init(NNLITE_MICRO_SPEECH_IRQ_PRIORITY);
		if (result != MTB_ML_RESULT_SUCCESS) {
			return -EIO;
		}

		/* buffer == NULL: let the middleware allocate its own
		 * persistent/scratch buffers sized from g_model_bin.arena_size. */
		result = mtb_ml_model_init(&g_model_bin, NULL, &g_model_obj);
		if (result != MTB_ML_RESULT_SUCCESS) {
			(void)mtb_ml_deinit();
			return -EINVAL;
		}
	...
	*input_scale = g_model_obj->input_scale;
	*input_zero_point = g_model_obj->input_zero_point;
	*output_scale = g_model_obj->output_scale;
	*output_zero_point = g_model_obj->output_zero_point;
...
int nnlite_micro_speech_infer(const int8_t *features, int8_t *scores_out)
{
	...
	if ((features == NULL) || (scores_out == NULL)) {
		return -EINVAL;
	}

	result = mtb_ml_model_run(g_model_obj, (MTB_ML_DATA_T *)features);
	if (result != MTB_ML_RESULT_SUCCESS) {
		return -EIO;
	}

	for (int i = 0; i < 4; i++) {
		scores_out[i] = (int8_t)g_model_obj->output[i];
	}
